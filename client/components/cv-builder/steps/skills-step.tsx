  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {currentCV.skills.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {currentCV.skills.map((skill, index) => (
            <Badge key={index} variant="secondary" className="gap-2 px-3 py-1.5 text-sm">
              {skill}
        ...
        <div className="rounded-lg border border-dashed border-border p-8 text-center">
          <p className="text-muted-foreground">No skills added yet. Start typing to add your skills.</p>
        </div>
      )}
      ...
      <div className="rounded-lg bg-muted p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {["JavaScript", "React", "Node.js", "Python", "Communication", "Leadership", "Problem Solving"].map(
            (suggestion) => (
              <Button
                key={suggestion}
                variant="outline"
